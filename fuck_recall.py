...
class FuckRecall(object):
    ...
    def ClearTimeOutMsg(self):
        if not len(self.msg_dict):
            return

        for msgid in list(self.msg_dict): # 由于字典在遍历过程中不能删除元素，故使用此方法
            if time.time() - self.msg_dict.get(msgid, None)["msg_time"] > 130.0: #超时两分钟
                item = self.msg_dict.pop(msgid)
    ...
    def getSavingMsg(self, msg, chatType):
        ...
        if msg['Type'] == 'Text':
            ...
            msg['Text'](FuckRecall.download_folder+msg['FileName'])
        elif msg['Type'] == 'Card':
    ...
    # 将已撤回的消息发给文件传输助手
    def SendRecalledMsg(self, old_msg):
        ...
        # 撤回消息是可保存的文件类型
            ...
            shutil.move(FuckRecall.download_folder+file_name, FuckRecall.recalled_file_folder)
            itchat.send(msg_send, toUserName='filehelper')  # 将撤回消息的通知以及细节发送到文件助手
            itchat.send(msg=FuckRecall.type_dict[old_msg['msg_type']]+'@recalled_file/'+file_name, toUserName='filehelper')
    ...
    @itchat.msg_register([TEXT, PICTURE, MAP, CARD, SHARING, RECORDING, ATTACHMENT, VIDEO, FRIENDS],
                         isFriendChat=True)
    def SaveFriendsMsg(self, msg):
        print 'received message!'
        msg_id = msg['MsgId']  # 消息ID

        saving_msg = self.getSavingMsg(msg, 'friendChat')
        # 更新字典
        # {msg_id:(msg_from,msg_time,msg_time_touser,msg_type,msg_content,msg_url)}
        self.msg_dict.update({msg_id: saving_msg})
        # 清理缓存消息列表
    ...
    @itchat.msg_register([TEXT, PICTURE, MAP, CARD, SHARING, RECORDING, ATTACHMENT, VIDEO, FRIENDS],
                         isGroupChat=True)
    def SaveGroupsMsg(self, msg):
        print 'received message!'
        msg_id = msg['MsgId']  # 消息ID

        saving_msg = self.getSavingMsg(msg, 'groupChat')
        # 更新字典
    ...
    def RecalledMsg(self, msg):
        # print(msg)
        print msg['Text']
        # 创建可下载消息内容的存放文件夹，并将暂存在当前目录的文件移动到该文件中
        if not os.path.exists(FuckRecall.download_folder):
            os.mkdir(FuckRecall.download_folder)
        ...
        if re.search(u'.*撤回了一条消息', msg['Text']) is not None:
            ...
            # 从暂存的消息列表中获取撤回的消息
            old_msg = self.msg_dict.get(old_msg_id)
            # 将撤回的消息发给自己
            self.SendRecalledMsg(old_msg)
            self.msg_dict.pop(old_msg_id)
            self.ClearTimeOutMsg()


if __name__ == '__main__':
    # 启动程序，并且设置二维码的保存路径
    itchat.auto_login(hotReload=True, enableCmdQR=True, picDir=FuckRecall.static_folder+'aaa.png')
    itchat.run()
